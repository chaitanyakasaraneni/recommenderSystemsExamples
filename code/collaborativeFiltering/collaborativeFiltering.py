...
def user_based_cf(user_id, item_id, matrix, k=10):
    ...
    # Predict rating
    numerator = sum(user_similarities[user_index][u] * matrix.iloc[u][item_id] for u in similar_users if not pd.isna(matrix.iloc[u][item_id]))
    denominator = sum(abs(user_similarities[user_index][u]) for u in similar_users if not pd.isna(matrix.iloc[u][item_id]))
...
def item_based_cf(user_id, item_id, matrix, k=10):
    ...
    # Predict rating
    numerator = sum(item_similarities[item_index][i] * matrix.loc[user_id, matrix.columns[i]] for i in similar_items if not pd.isna(matrix.loc[user_id, matrix.columns[i]]))
    denominator = sum(abs(item_similarities[item_index][i]) for i in similar_items if not pd.isna(matrix.loc[user_id, matrix.columns[i]]))
...
def get_recommendations(user_id, matrix, cf_function, n=5):
    ...
    predictions = [(item, cf_function(user_id, item, matrix)) for item in unrated_items]
...
user_based_recommendations = get_recommendations(user_id, user_item_matrix, user_based_cf)
...
item_based_recommendations = get_recommendations(user_id, user_item_matrix, item_based_cf)
...
sparsity = 1 - (ratings_df.shape[0] / (user_item_matrix.shape[0] * user_item_matrix.shape[1]))
